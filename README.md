...
**Ledgers for durable invariants.** Not all content belongs in a handoff. Handoffs can rot fast — particularly in large, multi-session tasks, where a conclusion ruled out early decays across the relay and a later session rederives it, or loses a fact already surfaced. A *ledger* is the home for that durable content: a persistent, per-project store of an effort's invariants — ruled-out approaches, framing decisions, surfaced facts — kept as a small living set that every session reads and maintains. The skill describes what belongs there and relays the *pointer*, not the contents, in each handoff's Ledgers section; a later session finds a ledger because the handoff it picked up named it, and reads it as live state before orienting. This replaces an earlier chaining mechanism (a `Continues from:` lineage line between handoffs) — across a long thread, the ledger is what actually needs carrying, not a breadcrumb trail back through consumed snapshots.
